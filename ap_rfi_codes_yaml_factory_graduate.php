...
$ap_schema = new Schema('../../../records/ap_rfi_codes_graduate');
$ap_table = $ap_schema->getTable();
print("__AP_Forms\n");
foreach( $ap_table as $key => $ap_row) {
  // column index 3 - is the program form paused or incomplete?
  if ($ap_row[3]==='FALSE' && !empty($ap_row[4])) {
    $url_arr = explode('?',$ap_row[1]);
    $form_path = basename($url_arr[0]);
    $query_arg = explode('=',$url_arr[1])[0];
    print(str_replace('_','-',$ap_row[0]) . "\n");
